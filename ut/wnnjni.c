...
#include "../libs/libwnnDictionary/OpenWnnJni.h"
#include "../libs/libwnnDictionary/predef_table.h"

char * GetStringUTFChars (char * in, int n) {
  return in;
}

void ReleaseStringUTFChars( char * a, const char* b ) {
...
char * NewStringUTF (const char * a) {
  return a;
}
...
static int convertStringToNjChar( NJ_CHAR* dst, char* srcJ, int maxChars )
{
	const unsigned char*	src;

	src = ( const unsigned char* )( GetStringUTFChars( srcJ, NULL ) );
	if( src != NULL ) {
		int     i, o;
		...
		ReleaseStringUTFChars( srcJ, ( const char* )src );
		return 0;
	}
...
static int convertNjCharToString( char** dstJ, NJ_CHAR* src, int maxChars )
{
  ...
  ( const char* dicLibPathJ)
{
	...
	if( work != NULL ) {
		...
		if( dicLibPathJ == NULL ||
			( dicLibPath = GetStringUTFChars( dicLibPathJ, 0 ) ) == NULL ) {
			free( work );
			return 0;
		}

		work->dicLibHandle = ( void* )dlopen( dicLibPath, RTLD_LAZY );
		ReleaseStringUTFChars( dicLibPathJ, dicLibPath );
 ...
 * Signature: (JIII)I
 */
 int setDictionaryParameter
  ( long wnnWork, int index, int base, int high)
{
	NJ_JNIWORK*	work;
 ...
 */
int searchWord
  ( long wnnWork, int operation, int order, char* keyString)
{
	NJ_JNIWORK*	work;
	...
	work = *( NJ_JNIWORK** )&wnnWork;
	if( work != NULL ) {
        if( GetStringLength( keyString ) > NJ_MAX_LEN ) {
		...
		if( convertStringToNjChar( work->keyString, keyString, NJ_MAX_LEN ) >= 0 ) {
            int    result;
  ...
  (  long wnnWork, int length)
{
	...
	if( work != NULL ) {
        if( work->flag & NJ_JNI_FLAG_ENABLE_CURSOR ) {
            ...
            if( length <= 0 ) {
        		result = ( int )njx_get_word( &( work->wnnClass ), &( work->cursor ), &( work->result ) );
            } else {
                do {
            		result = ( int )njx_get_word( &( work->wnnClass ), &( work->cursor ), &( work->result ) );
	...
	if( work != NULL ) {
        ...
        if( work->flag & NJ_JNI_FLAG_ENABLE_RESULT ) {
    		...
    		if( njx_get_stroke( &( work->wnnClass ), &( work->result ), stroke, sizeof( NJ_CHAR ) * ( NJ_MAX_LEN + NJ_TERM_LEN ) ) >= 0 &&
    			convertNjCharToString( &str, stroke, NJ_MAX_LEN ) >= 0 ) {
    			return str;
    		}
        } else {
            /* When njx_get_word() was not yet called, return "No result is found" */
            if( convertNjCharToString( &str, ( NJ_CHAR* )"\x00\x00", NJ_MAX_LEN ) >= 0 ) {
                return str;
            }
        }
	}
	...
	if( work != NULL ) {
        ...
        if( work->flag & NJ_JNI_FLAG_ENABLE_RESULT ) {
    		...
    		if( njx_get_candidate( &( work->wnnClass ), &( work->result ), candidate, sizeof( NJ_CHAR ) * ( NJ_MAX_RESULT_LEN + NJ_TERM_LEN ) ) >= 0 &&
    			convertNjCharToString( &str, candidate, NJ_MAX_RESULT_LEN ) >= 0 ) {
    			return str;
            }
        } else {
            /* When njx_get_word() was not yet called, return "No result is found" */
            if( convertNjCharToString( &str, ( NJ_CHAR* )"\x00\x00", NJ_MAX_RESULT_LEN ) >= 0 ) {
                return str;
            }
        }
	}
  ...
  ( long wnnWork)
{
	...
	if( work != NULL ) {
        if( work->flag & NJ_JNI_FLAG_ENABLE_RESULT ) {
    		return ( int )( work->result.word.stem.hindo );
        } else {
            /* When njx_get_word() was not yet called, return "No result is found" */
            return 0;
        }
 ...
 */
 int setApproxPattern__JLjava_lang_String_2Ljava_lang_String_2
  ( long wnnWork, char* srcJ, char* dstJ)
{
	NJ_JNIWORK*	work;

	if( srcJ == NULL || GetStringLength( srcJ ) == 0 || GetStringLength( srcJ ) > 1 ||
		dstJ == NULL || GetStringLength( dstJ ) == 0 || GetStringLength( dstJ ) > 3 ) {
		/* If a invalid parameter was specified, return an error code */
	...
	if( work != NULL ) {
		if( work->approxSet.charset_count < NJ_MAX_CHARSET ) {
			...
			if( convertStringToNjChar( from, srcJ, NJ_MAX_CHARSET_FROM_LEN ) >= 0 &&
				convertStringToNjChar( to, dstJ, NJ_MAX_CHARSET_TO_LEN )   >= 0 ) {
 ...
 * Signature: (JI)I
 */
 int setApproxPattern__JI
  (  long wnnWork, int approxPattern)
{
	NJ_JNIWORK	*work;
 ...
 * Signature: (JI)I
 */
 int setLeftPartOfSpeech
  (  long wnnWork, int leftPartOfSpeech)
{
	NJ_JNIWORK*	work;
 ...
 * Signature: (JI)I
 */
 int setRightPartOfSpeech
  (  long wnnWork, int rightPartOfSpeech)
{
	NJ_JNIWORK*	work;
  ...
  (  long wnnWork, char* stroke)
{
	...
	work = *( NJ_JNIWORK** )&wnnWork;
	if( work != NULL ) {
        if( GetStringLength( stroke ) > NJ_MAX_LEN ) {
    		/* If a invalid parameter was specified, return an error code */
        	return NJ_SET_ERR_VAL(NJ_FUNC_JNI_SET_STROKE, NJ_ERR_YOMI_TOO_LONG);
        }

        /* Store stroke string */
		if( convertStringToNjChar( work->previousStroke, stroke, NJ_MAX_LEN ) >= 0 ) {
            return 0;
        }
  ...
  (  long wnnWork, char* candidate)
{
	...
	work = *( NJ_JNIWORK** )&wnnWork;
	if( work != NULL ) {
        if( GetStringLength( candidate ) > NJ_MAX_RESULT_LEN ) {
    		/* If a invalid parameter was specified, return an error code */
        	return NJ_SET_ERR_VAL(NJ_FUNC_JNI_SET_CANDIDATE, NJ_ERR_CANDIDATE_TOO_LONG);
        }

        /* Store candidate string */
		if( convertStringToNjChar( work->previousCandidate, candidate, NJ_MAX_RESULT_LEN ) >= 0 ) {
            return 0;
        }
  ...
  (  long wnnWork)
{
	...
	if( work != NULL ) {
        ...
        memcpy( &( work->wnnClass.dic_set ), &( work->dicSet ), sizeof( NJ_DIC_SET ) );
        return ( int )njx_select( &( work->wnnClass ), &( work->result ) );
    }
 ...
 * Signature: (JI)I
 */
 int getLeftPartOfSpeechSpecifiedType
  (  long wnnWork, int type)
{
	NJ_JNIWORK*	work;
	...
	if( work != NULL ) {
        ...
        return ( int )njd_r_get_hinsi( work->dicSet.rHandle[ NJ_MODE_TYPE_HENKAN ], type );
    }
 ...
 * Signature: (JI)I
 */
 int getRightPartOfSpeechSpecifiedType
  (  long wnnWork, int type)
{
	NJ_JNIWORK*	work;
...
//
//        /* 1-origin */
//        resultJ = NewByteArray( rcount + 1 );
//
//        if( resultJ != NULL ) {
//            jbyte   *result;
//            result = GetByteArrayElements( resultJ, NULL );
//
//            if( result != NULL ) {
//                int         i;
//                NJ_UINT8*   connect;
...
//                }
//
//                ReleaseByteArrayElements( resultJ, result, 0 );
//                return resultJ;
//            }
//        }
 ...
 */
// jobjectArray getApproxPattern
//  (  long wnnWork, char* srcJ)
//{
//	NJ_JNIWORK*	work;
//
//	if( srcJ == NULL || GetStringLength( srcJ ) == 0 || GetStringLength( srcJ ) > 1 ) {
...
//            }
//
//            jclass strC = FindClass( "java/lang/String" );
//
//            if( strC != NULL ) {
//                jobjectArray retJ = NewObjectArray( outCount, strC, NULL );
//
//                if( retJ != NULL ) {
...
//                            char* dstJ;
//
//                            if( convertNjCharToString( &dstJ, work->approxSet.to[ i ], NJ_MAX_CHARSET_TO_LEN ) < 0 ) {
//                                return NULL;
//                            }
//
//                            SetObjectArrayElement( retJ, outIndex++, dstJ );
//                        }
//
//                    }
//                    return retJ;
 ...
 */
// jobjectArray createBindArray
//  (  long wnnWork, char* keyStringJ, int maxBindsOfQuery, int maxPatternOfApprox)
//{
//	NJ_JNIWORK*	work;
//
//	if( keyStringJ == NULL ) {
...
//
//        if( strC != NULL ) {
//            jobjectArray retJ = NewObjectArray( maxBindsOfQuery * (maxPatternOfApprox+1), strC, NULL );
//
//            if( retJ != NULL ) {
//                NJ_CHAR     keyString[ NJ_MAX_LEN + NJ_TERM_LEN ];
//
//                if( convertStringToNjChar( keyString, keyStringJ, NJ_MAX_LEN ) >= 0 ) {
...
//
//                                char* dstJ;
//                                if( convertNjCharToString( &dstJ, baseStr, NJ_MAX_LEN ) < 0 ) {
//                                    return NULL;
//                                }
//
//                                SetObjectArrayElement( retJ, outIndex++, dstJ );
 ...
 */
 char* createQueryStringBase
  (  long wnnWork, int maxBindsOfQuery, int maxPatternOfApprox, char* keyColumnNameJ)
{
    NJ_JNIWORK*	work;
    char* retJ = NULL;
	...
	work = *( NJ_JNIWORK** )&wnnWork;
	if( work != NULL ) {
    	const unsigned char* keyName = ( const unsigned char* )( GetStringUTFChars( keyColumnNameJ, NULL ) );
        ...
        if( keyName != NULL ) {
            ...
            if( dst != NULL ) {
                ...
                dst[ dstPtr ] = '\0';
                retJ = NewStringUTF( dst );

                free( dst );
            }

       		ReleaseStringUTFChars( keyColumnNameJ, ( const char* )keyName );
        }
    }
	return retJ;
}
